...
public class GemstonePodium : MonoBehaviour
{
    ...
    private void Update()
    {
        if(currentGemstone != null && (currentGemstone.transform.position - transform.position).magnitude < 0.5f && !activated)
        {
            activated = true;
    ...
    {
        if(collision.gameObject.TryGetComponent(out Gemstone gemstone) && gemstone.GetColorCode() == color && !activated)
        {
